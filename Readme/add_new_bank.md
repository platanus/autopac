Ir a /src/active_pages.json y añadir el nuevo banco con sus respectivos atributos, con esto ya debería ser posible detectar y activar la extensión en la página del nuevo banco.
...
Por último es necesario volver al manifest y agregar los match y los scripts que necesita la página. Dentro de content script es necesario agregar el match para el banco con su respectivo inject, luego se debe agregar el <banco>_fun.js dentro de web_accesible_resources, para que la pagina pueda acceder a las funciones insertadas desde la extensión.
